"""Diagnostic chen-so (spec 05 V2): so flip-rate cua cac specificity scorer.
  ...
  python experiments/diagnose_specificity.py [--n 40] [--scorers digit,llm]
...
rubric) ky vong flip ~0. Ket qua -> experiments/metrics/specificity_diagnostic.json.
"""
import argparse
import json
import sys
...
GOLD = Path("source_data/translate/specificity.test.parquet")
...
def main(argv=None):
    ...
    out = Path("experiments/metrics/specificity_diagnostic.json")
