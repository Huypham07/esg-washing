...
config.json (chua heads/thresholds). Repo tai ve can code esgwash de load.
  ...
  python scripts/upload_to_hf.py outputs/models/topic/vi --repo <user>/esgwash-topic-vi
  python scripts/upload_to_hf.py outputs/models/claim/vi --repo <user>/esgwash-claim-vi --private

Cung dung de CHI convert model.pt cu -> safetensors (khong upload):
  python scripts/upload_to_hf.py outputs/models/claim/vi --convert-only
"""
import argparse
import json
import os
