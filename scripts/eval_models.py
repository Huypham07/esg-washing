"""Danh gia 2 model (topic, claim) tren test — bao boc tach per-head + visualize.
...
Topic test = GOLD thuan (o non-gold = NaN -> tu mask). Claim test = climatebert chuan.
  ...
  python scripts/eval_models.py            # ca 2
  python scripts/eval_models.py --only topic
"""
import argparse
import json
import sys
...
REPOS = {"topic": "huypham71/esg-topic", "claim": "huypham71/esg-claim"}
TEST = {"topic": "data/topic_test.parquet", "claim": "data/claim_test.parquet"}
BASELINE = {"topic": 0.813, "claim": 0.655}  # TF-IDF+LR macro-F1 (run_baselines)
