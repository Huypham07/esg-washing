...
def is_noise_sentence(s: str, section_title: str = "") -> bool:
    ...
    # Blob bảng / câu ghép lỗi: quá dài, hoặc chứa xuống dòng / pipe nhúng giữa câu.
    ...
    # Mảnh số liệu bảng: tỉ lệ chữ số cao.
...
def build(
    ...
    df_sents = df_sents.drop_duplicates(subset=["sentence"]).reset_index(drop=True)
