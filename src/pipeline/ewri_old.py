...
def calculate_bank_year_ewri(df: pd.DataFrame) -> list[EWRIScore]:
    ...
    for (bank, year), group in df.groupby(["bank", "year"]):
        ...
        # Actionability counts
        implemented = (group["action_label"] == "Implemented").sum()
        planning = (group["action_label"] == "Planning").sum()
        indeterminate = (group["action_label"] == "Indeterminate").sum()
...
def calculate_topic_ewri(
    ...
    for topic in esg_topics:
        ...
        implemented = (topic_df["action_label"] == "Implemented").sum()
        indeterminate = (topic_df["action_label"] == "Indeterminate").sum()
...
def run(input_path: Path = INPUT_PATH, output_dir: Path = OUTPUT_DIR):
    ...
    if scores:
        for claim in scores[0].top_risk_claims[:5]:
            print(f"  [{claim['risk_score']:.1f}] [{claim['action_label']}] "
