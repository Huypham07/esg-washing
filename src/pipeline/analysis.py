...
def analyze_evidence_types(df: pd.DataFrame) -> dict:
    ...
    for etype in VALID_EVIDENCE_TYPES:
        ...
        has_type = df["evidence_types"].apply(
            lambda x: etype in (x.tolist() if isinstance(x, np.ndarray) else x) if isinstance(x, (list, np.ndarray)) else False
        )
    ...
    for t1 in VALID_EVIDENCE_TYPES:
        for t2 in VALID_EVIDENCE_TYPES:
            ...
            has_both = df["evidence_types"].apply(
                lambda x: t1 in (x.tolist() if isinstance(x, np.ndarray) else x) and t2 in (x.tolist() if isinstance(x, np.ndarray) else x) if isinstance(x, (list, np.ndarray)) else False
            )
    ...
    def _extract_samples(mask, label, n=n_per_category):
        ...
        for _, r in sub.head(n).iterrows():
            rows.append({
                ...
                "evidence_types": list(r.get("evidence_types", [])) if isinstance(r.get("evidence_types", []), (list, np.ndarray)) else [],
        ...
        for cat_key, cat_name in [
            ...
            if samples:
                ...
                for i, s in enumerate(samples[:3], 1):
                    ...
                    et = s.get("evidence_types", [])
                    if isinstance(et, (list, np.ndarray)) and len(et) > 0:
                        lines.append(f"   Evidence types: {', '.join(str(t) for t in et)}")
